    ...
    <section className="instagram">
      <div className="container">
        <div className="instagram__header">
          <h2 className="instagram__title">{title}</h2>
          <a href={linkUrl} className="instagram__link" target="_blank" rel="noreferrer">
            <span>Подпишись</span>
        ...
        <div className="instagram__grid">
          {posts.map((post) => (
            <div key={post.id} className="instagram__item">
              <img src={post.img} alt={post.alt || 'Instagram photo'} />
              <div className="instagram__overlay">
                <span>{post.overlayText || accountHandle}</span>
              </div>
            </div>
          ))}
        </div>
